...
class SimulationPlotter:
    ...
    def plot_energy_degradation(results_dict: Dict, output_dir: str, max_trajectories: int = 50):
        """Crea grafico della degradazione energetica - VERSIONE MIGLIORATA"""
        # Seleziona fino a 4 umidità rappresentative tra quelle disponibili
        available_humidities = sorted(results_dict.keys())
        
        if len(available_humidities) == 1:
            selected_humidities = [available_humidities[0]]
            subplot_config = (1, 1)
        elif len(available_humidities) == 2:
            selected_humidities = available_humidities
            subplot_config = (1, 2)
        elif len(available_humidities) == 3:
            selected_humidities = available_humidities
            subplot_config = (2, 2)  # Usa 4 subplot ma il 4° rimarrà vuoto
        else:
            # Seleziona 4 umidità ben distribuite
            if len(available_humidities) >= 4:
                indices = [0, len(available_humidities)//3, 2*len(available_humidities)//3, -1]
                selected_humidities = [available_humidities[i] for i in indices]
            else:
                selected_humidities = available_humidities
            subplot_config = (2, 2)
        
        # Colori fissi per consistenza
        color_map = {0: 'red', 1: 'orange', 2: 'green', 3: 'blue'}
        
        fig, axes = plt.subplots(subplot_config[0], subplot_config[1], figsize=(15, 12))
        
        # Gestisci caso con un solo subplot
        if len(selected_humidities) == 1:
            axes = [axes]
        else:
            axes = axes.flatten() if hasattr(axes, 'flatten') else axes
        
        for idx, humidity in enumerate(selected_humidities):
            if humidity in results_dict:
                result = results_dict[humidity]
                ax = axes[idx] if len(selected_humidities) > 1 else axes[0]
        ...
        if len(selected_humidities) < 4 and subplot_config == (2, 2):
            for idx in range(len(selected_humidities), 4):
    ...
    @staticmethod
    def plot_efficiency_comparison(results_dict: Dict, output_dir: str, h1: float, h2: float):
        """Crea grafico di confronto delle efficienze tra due umidità"""
        humidities = [h1, h2]
        efficiencies = []
        colors = ['crimson', 'steelblue']
        
        for h in humidities:
            result = results_dict[h]
            eff = result['thermalized_neutrons'] / result['initial_neutrons']
            efficiencies.append(eff)
        
        plt.figure(figsize=(10, 6))
        bars = plt.bar(range(len(humidities)), efficiencies, color=colors, alpha=0.8, width=0.6)
        
        # Aggiungi valori sopra le barre
        for i, (bar, eff) in enumerate(zip(bars, efficiencies)):
            height = bar.get_height()
            plt.text(i, height + height*0.01, f'{eff:.4f}', 
                    ha='center', va='bottom', fontweight='bold', fontsize=12)
        
        plt.xlabel('Soil Moisture (%)', fontsize=12)
        plt.ylabel('Thermalization Efficiency', fontsize=12)
        plt.title(f'Efficiency Comparison: {h1}% vs {h2}%', fontsize=14)
        plt.xticks(range(len(humidities)), [f'{h}%' for h in humidities])
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        
        filepath = os.path.join(output_dir, f'efficiency_comparison_{h1}%_vs_{h2}%.png')
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
        return filepath
    
    @staticmethod
    def plot_depth_comparison(results_dict: Dict, output_dir: str, h1: float, h2: float):
        """Crea grafico di confronto delle distribuzioni di profondità"""
        plt.figure(figsize=(12, 8))
        ...
        for i, h in enumerate(humidities):
            ...
            if result['thermalization_depths']:
                depths = result['thermalization_depths']
                plt.hist(depths, bins=50, alpha=0.7, color=colors[i], 
                        label=f'{h}%', density=True, range=(0, 100))
        
        plt.xlabel('Thermalization Depth (cm)', fontsize=12)
        plt.ylabel('Probability Density', fontsize=12)
        plt.title(f'Depth Distribution Comparison: {h1}% vs {h2}%', fontsize=14)
        plt.legend(title='Humidity', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.xlim(0, 100)
        plt.tight_layout()
        
        filepath = os.path.join(output_dir, f'depth_comparison_{h1}%_vs_{h2}%.png')
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
        return filepath
    
    @staticmethod
    def plot_energy_trajectories_comparison(results_dict: Dict, output_dir: str, h1: float, h2: float, max_trajectories: int = 30):
        """Crea grafico di confronto delle traiettorie energetiche"""
        plt.figure(figsize=(12, 8))
        ...
        for i, h in enumerate(humidities):
            ...
            for trajectory in energy_histories:
                if len(trajectory) > 1:
                    plt.plot(trajectory, alpha=0.6, color=colors[i], linewidth=1, 
                            label=f'{h}%' if trajectory == energy_histories[0] else "")
        
        plt.xlabel('Collision Number', fontsize=12)
        plt.ylabel('Energy (eV)', fontsize=12)
        plt.yscale('log')
        plt.title(f'Energy Trajectories Comparison: {h1}% vs {h2}%', fontsize=14)
        plt.grid(True, alpha=0.3)
        
        # Crea legenda manuale per evitare troppi elementi
        from matplotlib.lines import Line2D
        legend_elements = [Line2D([0], [0], color=colors[0], lw=2, label=f'{h1}%'),
                          Line2D([0], [0], color=colors[1], lw=2, label=f'{h2}%')]
        plt.legend(handles=legend_elements, title='Humidity', fontsize=12)
        
        plt.tight_layout()
        
        filepath = os.path.join(output_dir, f'energy_trajectories_comparison_{h1}%_vs_{h2}%.png')
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
        return filepath
...
class MenuSystem:
    ...
    def create_plots_menu(self):
        """Menu per la creazione dei grafici - VERSIONE MIGLIORATA"""
        ...
        while True:
            ...
            print("5. Grafici confronto tra 2 umidità (separati)")
            print("0. Torna indietro")
    ...
    def create_all_plots(self):
        """Crea tutti i grafici disponibili - VERSIONE MIGLIORATA"""
        ...
        try:
            ...
            # Degradazione energetica (adattiva)
            filepath3 = self.plotter.plot_energy_degradation(
                self.results_cache, self.config.output_dir)
            print(f"✓ Creato: {filepath3}")
            created_files.append(filepath3)
            
            # Confronti tra la prima e ultima umidità (se ce ne sono almeno 2)
            humidities = sorted(self.results_cache.keys())
            if len(humidities) >= 2:
                h1, h2 = humidities[0], humidities[-1]
                
                # Confronto efficienza
                filepath4 = self.plotter.plot_efficiency_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath4}")
                created_files.append(filepath4)
                
                # Confronto profondità
                filepath5 = self.plotter.plot_depth_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath5}")
                created_files.append(filepath5)
                
                # Confronto traiettorie energetiche
                filepath6 = self.plotter.plot_energy_trajectories_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath6}")
                created_files.append(filepath6)
            
            print(f"\nTutti i grafici sono stati creati con successo!")
            print(f"Totale file creati: {len(created_files)}")
    ...
    def create_comparison_plots(self):
        """Crea grafici di confronto tra due umidità - VERSIONE MIGLIORATA"""
        humidities = sorted(self.results_cache.keys())
        if len(humidities) < 2:
        ...
        try:
            ...
            if h1 in self.results_cache and h2 in self.results_cache:
                print("Creazione dei grafici di confronto...")
                created_files = []
                
                # Confronto efficienza
                filepath1 = self.plotter.plot_efficiency_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath1}")
                created_files.append(filepath1)
                
                # Confronto profondità
                filepath2 = self.plotter.plot_depth_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath2}")
                created_files.append(filepath2)
                
                # Confronto traiettorie energetiche
                filepath3 = self.plotter.plot_energy_trajectories_comparison(
                    self.results_cache, self.config.output_dir, h1, h2)
                print(f"✓ Creato: {filepath3}")
                created_files.append(filepath3)
                
                print(f"Confronto completato! Creati {len(created_files)} grafici.")
            else:
    ...
    def show_help(self):
        ...
        print("MIGLIORAMENTI VERSIONE CORRENTE:")
        print("• Grafici di confronto separati per maggiore chiarezza")
        print("• Grafico degradazione energetica adattivo alle umidità disponibili")
        print("• Menu grafici migliorato con opzioni separate")
        print("• Migliore gestione dei colori e layout")
        print()
        ...
        print("TIPI DI GRAFICI DISPONIBILI:")
        print("• Efficienza termalizzazione: confronto tra tutte le umidità")
        print("• Distribuzioni profondità: curve sovrapposte per confronto")
        print("• Degradazione energetica: 1-4 subplot adattivi")
        print("• Confronti separati: efficienza, profondità, traiettorie")
        print()
        ...
        print("• Grafici vuoti: verifica che le simulazioni siano completate")
